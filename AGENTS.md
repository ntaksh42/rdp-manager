...
### Session overview (Views/SessionOverviewWindow + Controls/SessionSnapshot)
Ctrl+Alt+0 shows every open session as a thumbnail grid. It is an **owned borderless window laid over the main window's client area**, not a WPF overlay — WPF can't draw above the embedded RDP HWNDs (airspace). Thumbnails come from `RdpSessionControl.TryUpdateSnapshot` (`PrintWindow` with `PW_RENDERFULLCONTENT`, downscaled to ≤1600px and frozen): a session is captured right before it is hidden on tab switch (conversion deferred to Background priority so switching stays fast), foreground sessions are captured when the overview opens. **Never capture a hidden session**: `PrintWindow(PW_RENDERFULLCONTENT)` on a hidden child HWND returns the top-level window's composed pixels at that rect — i.e. the foreground session — so every tile would show the same screen (shipped once in v0.4.8). Instead, while the overview covers the main window, it round-robins every connected session every 600ms: `SessionManager.ShowForCapture` makes only that session visible in its pane, the next tick captures it (giving the control time to repaint), and `RestoreVisibility` puts the selected tabs back when the overview closes. A blank/black capture keeps the previous snapshot. Tile sizing is pure logic in `Common/OverviewLayout` (picks the column count that maximizes tile size; unit-tested).
...
Light/dark is implemented by swapping frozen brushes in `Application.Current.Resources` at runtime. **Never hardcode a color in XAML or code** — use `{DynamicResource X}` with the existing keys (`WindowBg`, `PanelBg`, `TextFg`, `SubtleFg`, `BarBg`, `SplitterBg`, `ControlBg`, `ControlBorder`, `TabBg`, `TabSelectedBg`, `HoverBg`, `PressedBg`, `AccentBg`/`AccentFg` for selection emphasis); a `StaticResource` or literal color will not follow the theme toggle. App.xaml holds themed implicit styles for common controls and dialogs.
