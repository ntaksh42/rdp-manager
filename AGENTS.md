...
Ctrl+Alt+0 shows every open session as a thumbnail grid. It is an **owned borderless window laid over the main window's client area**, not a WPF overlay — WPF can't draw above the embedded RDP HWNDs (airspace). Thumbnails come from `RdpSessionControl.TryUpdateSnapshot` (`PrintWindow` with `PW_RENDERFULLCONTENT`, downscaled to ≤1600px and frozen): a session is captured right before it is hidden on tab switch (conversion deferred to Background priority so switching stays fast), foreground sessions are captured when the overview opens and every 2s while it is open. **Never capture a hidden session**: `PrintWindow(PW_RENDERFULLCONTENT)` on a hidden child HWND returns the top-level window's composed pixels at that rect — i.e. the foreground session — so every tile would show the same screen (shipped once in v0.4.8). Hidden tabs show the snapshot taken when they were hidden; a blank/black capture keeps the previous snapshot. Tile sizing is pure logic in `Common/OverviewLayout` (picks the column count that maximizes tile size; unit-tested).
