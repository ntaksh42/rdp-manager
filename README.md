...
- 接続は Windows 標準 `mstsc.exe` を起動（OS 純正の RDP スタックを使用）。資格情報は `cmdkey` 経由で Windows 資格情報マネージャーへ注入し、`.rdp` に平文パスワードを書きません
...
- 接続時、`cmdkey` 実行の一瞬だけパスワードがコマンドライン引数に乗ります。共有マシンでの利用時はご注意ください
